...
public class AutoBeatMapGenerator : MonoBehaviour
{
    ...
    void AssignObstacles(List<NoteData> notes)
    {
        ...
        Debug.Log($"🚧 목표 장애물 개수: {targetObstacleCount}개 ({obstacleRatio * 100:F1}%)");
        ...
        while (obstacleCount < targetObstacleCount && availableIndices.Count > 0)
        {
            int randomIndex = Random.Range(0, availableIndices.Count);
            int noteIndex = availableIndices[randomIndex];

            float noteTime = notes[noteIndex].time;

            // 최소 간격 체크
            if (noteTime - lastObstacleTime >= minInterval)
            {
                notes[noteIndex].type = "obstacle";
                lastObstacleTime = noteTime;
                obstacleCount++;
            }
        ...
        Debug.Log($"✅ 장애물 {obstacleCount}개 배치 완료");
    }
